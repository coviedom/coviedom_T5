...
/* Estructura que contiene la configuración minima necesaria para el manejo del */
typedef struct
{
    uint8_t             TIMx_mode;              // Up or dowm
    uint16_t            TIMx_Prescaler;         // Prescaler....
    uint32_t            TIMx_Period;            // Valor en ms del periodo del Timer
    uint8_t             TIMx_InterruptEnable;   // Activa o desactiva el modo interrupcines
} Timer_BasicConfig_t;
